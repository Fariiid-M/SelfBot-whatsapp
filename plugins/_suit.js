  ...
  if (room) {
    ...
    if (m.sender == room.p2 && /^(acc(ept)?|terima|gas|oke?|tolak|gamau|nanti|ga(k.?)?bisa)/i.test(m.text) && m.isGroup && room.status == 'wait') {
      if (/^(tolak|gamau|nanti|ga(k.?)?bisa)/i.test(m.text)) {
        this.reply(m.chat, `@${room.p2.split`@`[0]} menolak suit, suit dibatalkan. Cupu abiezz..`, m)
...
_Silahkan klik link sesuai pilihan anda di bawah ini_
*Batu >* wa.me/${conn.user.jid.split`@`[0]}?text=batu
*Gunting >* wa.me/${conn.user.jid.split`@`[0]}?text=gunting
*Kertas >* wa.me/${conn.user.jid.split`@`[0]}?text=kertas`, m.chat, {
        contextInfo: {
          mentionedJid: [room.p, room.p2]
        }
      })

      if (!room.pilih) this.send3Button(room.p, 'Silahkan pilih', ``, 'Batu🗿', 'Batu', 'Kertas📄', 'Kertas', 'Gunting✂️', 'Gunting', m)
      if (!room.pilih2) this.send3Button(room.p2, 'Silahkan pilih', ``, 'Batu🗿', 'Batu', 'Kertas📄', 'Kertas', 'Gunting✂️', 'Gunting', m)
